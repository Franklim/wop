module.exports = Object.freeze({
    
    TABLE_GROUPS:               'groups',
    TABLE_ORDERS_STATES:        'orders_states',
    TABLE_PERSONS:              'persons',
    TABLE_STATES:               'states',
    TABLE_USERS:                'users',
    TABLE_PAYMENT_CONDITIONS:   'payment_conditions',
    TABLE_PRODUCTS:             'products',

    GROUP_POST:     "/groups",
    GROUP_PUT:      "/groups/:id",
    GROUP_GET:      "/groups",
    GROUP_DELETE:   "/groups/:id",

    PERSON_POST:     "/persons",
    PERSON_PUT:      "/persons/:id",
    PERSON_GET:      "/persons",
    PERSON_DELETE:   "/persons/:id",

    USER_POST:     "/users",
    USER_PUT:      "/users/:id",
    USER_GET:      "/users",
    USER_DELETE:   "/users/:id",

    CONDITION_POST:     "/conditions",
    CONDITION_PUT:      "/conditions/:id",
    CONDITION_GET:      "/conditions",
    CONDITION_DELETE:   "/conditions/:id",

    PRODUCT_POST:     "/products",
    PRODUCT_PUT:      "/products/:id",
    PRODUCT_GET:      "/products",
    PRODUCT_DELETE:   "/products/:id",

    AUTH_POST:      "/auth"

});